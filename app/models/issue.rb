...
class Issue < ActiveRecord::Base
  ...
  after_save :send_sms, if: :saved_change_to_status_id?
  ...
  def handle_send_sms noticeArr, sms_status
    checkExist = true
    if sms_status == 1
      pm_members = Member.joins(:member_roles).includes(:user)
        .where(member_roles: {role_id: 7} , project_id: project_id)
      pm_members.each do |member|
        user = member.user
        next unless user
        user_sms = ::VecoPhone.where(name: user.login)
        RequirementMailer.handle_sendSMS(user_sms, noticeArr).deliver
        checkExist = member.user_id != author_id
      end
    end
    return unless assigned_to
    if checkExist || status == 2
      user_sms = ::VecoPhone.where(name: assigned_to.login)
      RequirementMailer.handle_sendSMS(user_sms, noticeArr).deliver
    end
  end
  ...
  def send_sms
    case status_id
    when 15
      handle_send_sms ["[VECO] Kính gửi đồng chí, Đ/c có nhiệm vụ số: #{id} đang chờ phê duyệt. Đ/c truy cập địa chỉ http://plm.vht.vn/issues/#{id} để xử lý !"], 1
    when 11
      handle_send_sms ["[VECO] Kính gửi đồng chí, nhiệm vụ số: #{id} đã được phê duyệt. Đ/c truy cập địa chỉ http://plm.vht.vn/issues/#{id} để xem chi tiết !"], 2
    when 12
      handle_send_sms ["[VECO] Kính gửi đồng chí, nhiệm vụ số: #{id} bị từ chối phê duyệt. Đ/c truy cập địa chỉ http://plm.vht.vn/issues/#{id} để xử lý !"], 2
    end
  end
end
