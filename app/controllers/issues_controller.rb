...
class IssuesController < ApplicationController
    ...
    if @issue.save
      ...
      respond_to do |format|
        format.html {
          ...
          #start_copy
          noticeArr =[]
          noticeArr.push("[VECO] Đồng chí đã được giao nhiệm vụ với mã: #{@issue.id} thuộc dự án: #{@issue.project[:name]}. Đ/c truy cập địa chỉ http://plm.vht.vn/issues/#{@issue.id} để xử lý !")
          (User.where(id:@issue.assigned_to_id)).each do |getUser|
             @userSMS = VecoPhone.where(name: getUser.login)
             RequirementMailer.handle_sendSMS(@userSMS, noticeArr).deliver
          end
          #end_copy
        }
  ...
  def update
    if  params[:issue][:status_id].to_i == 11
          ...
          url = 'http://10.55.56.3:8081/redmine-jbpm-intergration/api/v1/jbpm/task/status?issueid=' + @issue.id.to_s
